...
function AdminPage() {
    ...
    const findRestaurantById = (restaurants, type, id) => {
        let restaurantId = null;
        if (type === 'dishId') {
            restaurantId = restaurants.find(restaurant => restaurant.dishes.some(dish => dish === id));
        } else if (type === 'tableId') {
            restaurantId = restaurants.find(restaurant => restaurant.tables.some(table => table === id));
        }
        return restaurantId ? restaurantId._id : null;
    };
    ...
    const handleDishDelete = async (dishId) => {
        try {
            ...
            const restaurantId = findRestaurantById(restaurants, 'dishId', dishId);
    ...
    const handleTableDelete = async (tableId) => {
        try {
            await fetchRestaurants(); // Дождаться обновления restaurants
            const restaurantId = findRestaurantById(restaurants, 'tableId', tableId);
            if (restaurantId) {
    ...
    const fetchDishes = () => {
        fetchData('http://localhost:3002/dish', setDishes);
    };

    const fetchRestaurants = () => {
        fetchData('http://localhost:3002/restaurant', setRestaurants);
    };

    const fetchTables = () => {
        fetchData('http://localhost:3002/table', setTables);
    };

    useEffect(() => {
        fetchData('http://localhost:3002/order/many', setOrders);
        fetchDishes();
        fetchRestaurants();
        fetchTables();
    }, []);
    ...
    const CustomDialog = (props) => {
        ...
        const {openFunc, closeFunc, deleteFunc, name} = props;

        return (
            <Dialog open={openFunc} onClose={closeFunc}>
                <DialogTitle>Confirmation</DialogTitle>
                <DialogContent>
                    <Typography>Are you sure you want to delete this {name ? name : null}?</Typography>
                </DialogContent>
                <DialogActions>
                    <Button onClick={closeFunc} color="primary">
                        Cancel
                    </Button>
                    <Button onClick={deleteFunc} color="primary" autoFocus>
                        Delete
                    </Button>
                </DialogActions>
            </Dialog>
        ...
        return (
            <TabPanel value={value} index={1}>
                <div>
                    ...
                    <CustomDialog openFunc={deleteConfirmationOpen} closeFunc={closeDeleteConfirmation} deleteFunc={confirmDelete} name='dish'/>
                </div>
            </TabPanel>
        )
    }
        ...
        return (
            <TabPanel value={value} index={2}>
                <div>
                    ...
                    <CustomDialog openFunc={deleteConfirmationOpenRest} closeFunc={closeDeleteConfirmationRest} deleteFunc={confirmDeleteRest} name='restaurant'/>
        ...
        return (
            <TabPanel value={value} index={3}>
                <div>
                    ...
                    <CustomDialog openFunc={deleteConfirmationOpenTable} closeFunc={closeDeleteConfirmationTable} deleteFunc={confirmDeleteTable} name='table'/>
                </div>
            </TabPanel>
        )
    }
